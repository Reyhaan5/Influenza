...
export default function UnifiedChatApp({ role = "influencer" }) {
  ...
  const brandList = useMemo(() => {
    ...
    const defaultName = user?.company || user?.name || (role === "brand" ? "My Brand" : "Collabs");
  ...
  // Fetch Brands if user is a brand
  useEffect(() => {
    if (role === "brand") {
      axios
        .get(`${API_URL}/brand/brands`, authHeader)
  ...
  const otherParticipant = (c) => {
    return c.participants?.find((p) => p._id !== user?.id) || c.participants?.[0];
  };
  ...
  return (
    <div className="flex h-screen w-full bg-white overflow-hidden">
      {/* LEFT COLUMN: CHATS / INBOX LIST (Width ~320px) */}
      <div className="w-80 border-r border-gray-200 flex flex-col h-full bg-white shrink-0">
        ...
        <div className="p-3 border-b border-gray-100 space-y-2.5">
          {/* Row 1: Brand / Category Pill + Circular Search Button */}
                ...
                <div className="absolute left-0 top-full mt-1.5 w-72 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 p-2.5 animate-fadeIn">
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-0.5 py-0.5">
                    {filteredBrandList.length === 0 ? (
                      <p className="text-xs text-gray-400 py-1.5 px-2">No partners found</p>
                    ) : (
          ...
          {searchOpen && (
            <div className="relative animate-fadeIn">
              <input
                type="text"
                placeholder={role === "brand" ? "Search by creator name..." : "Search by brand name..."}
                value={searchQuery}
              ...
              return (
                <div
                  ...
                  className={`group w-full p-3.5 flex items-start gap-3 cursor-pointer transition-colors ${
                    isSelected
                      ? "bg-gray-50 border-l-2 border-zinc-950 font-bold"
                      : "hover:bg-gray-50"
                  }`}
                >
                    ...
                    <div className="flex items-center justify-between mb-0.5">
                      <p
                        className={`text-xs truncate ${
                          isSelected ? "text-gray-950 font-bold" : "text-gray-900 font-semibold"
                        }`}
                      >
                        {other?.name || (role === "brand" ? "Creator" : "Brand Representative")}
                      </p>
                      {formattedTime && (
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 rounded text-gray-400 hover:text-red-600 transition cursor-pointer"
                    title="Delete chat"
                  >
                    <Trash2 size={13} />
                  </button>
          ...
          <>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-xs font-bold text-gray-900 truncate">
                    {activeOther?.name || (role === "brand" ? "Creator" : "Brand Partner")}
                  </h2>
                  <p className="text-[10px] text-gray-500 truncate">
                    {activeOther?.email || (role === "brand" ? "Creator Collaboration" : "Brand Collaboration")}
                  </p>
                </div>
              </div>
                    ...
                    >
                      <div
                        className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-xs leading-relaxed ${
                          mine
                            ? "bg-zinc-900 text-white rounded-br-xs shadow-xs font-medium"
            ...
            >
              ...
              <button
                ...
                className="p-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-white disabled:opacity-40 transition shadow-xs cursor-pointer"
              >
                <Send size={15} />
              </button>
            </form>
